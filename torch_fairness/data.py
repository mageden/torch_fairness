...
from torch_fairness.exceptions import (
    DummyCodingError,
    MissingSensitiveGroupsError,
    NotFittedError,
)
    ...
    def __init__(self, *args: Union[dict, SensitiveAttribute]):
        ...
        for i in args:
            if isinstance(i, dict):
                ...
                raise ValueError(
                    "Expected args to be either a dict or SensitiveAttribute."
                )
            if i.majority in i.minority:
                raise ValueError(
                    f"SensitiveAttribute {i.name} cannot have majority group in minority groups."
                )
            attributes.append(i)
    ...
    def infer(cls, x: Union[pd.DataFrame, np.ndarray], minimum_sample_size: int = 30):
        ...
        for col_i, colname in enumerate(colnames):
            ...
            if len(sufficient_counts) < 2:
                raise ValueError(
                    f"{col_i} column has less than two groups satisfying minimum sample size "
            ...
            group = SensitiveAttribute(
                majority=majority, minority=minority, name=colname
            )
...
class SensitiveTransformer:
    ...
    def __init__(
        self, sensitive_map: SensitiveMap = None, minimum_sample_size: int = 30
    ):
        if not isinstance(minimum_sample_size, int) or minimum_sample_size <= 0:
            raise ValueError(
                "minimum_sample_size should be a positive non-zero integer."
            )
    ...
    def fit(self, x: Union[pd.DataFrame, np.ndarray]):
        ...
        if self.sensitive_map is None:
            ...
            self.sensitive_map = SensitiveMap.infer(
                x, minimum_sample_size=self.minimum_sample_size
            )
        else:
            ...
            for col_i, sensitive_attribute in enumerate(self.sensitive_map):
                ...
                if len(group_diff) != 0:
                    raise ValueError(
                        f"Expected groups for {sensitive_attribute.name} not found - {group_diff}"
                    )
                matched_groups = [
                    i
                    for i in groups
                    if group_counts.get(i, 0) >= self.minimum_sample_size
                ]
                if len(matched_groups) != len(groups):
                    raise ValueError(
                        f"Groups failed to meet minimum_sample_size {self.minimum_sample_size}: "
    ...
    def inverse_transform(
        ...
        for i, (maj, min) in enumerate(self.sensitive_map.majority_minority_pairs):
            ...
            if (all_missing != missing.any(axis=1)).any():
                raise DummyCodingError(
                    f"Partial missing was observed for {self.sensitive_map[i].name}. If one group is missing they all should be."
                )
            if (np.nan_to_num(x[:, combined_idx].sum(axis=1), nan=1) != 1).any():
                raise MissingSensitiveGroupsError(
                    f"Multiple group membership within label {self.sensitive_map[i].name} were found - groups should be non-overlapping within an attribute."
                )
            group = x[:, combined_idx].argmax(1).astype(np.float32)
            group = np.array(
                [group_names[x] for x in group], dtype=self._sensitive_dtype[i]
            )
    ...
    def _validate_data(
        self, x: Union[pd.DataFrame, np.ndarray], is_dummy_coded: bool
    ) -> None:
    ...
    def _validate_datatype(
        ...
        if is_dummy_coded:
            ...
            correct = np.all((x == 0.0) | (x == 1.0) | pd.isnull(x))
            if not correct:
                raise DummyCodingError(
                    "All values should be either 1., 0., or nan - encountered unexpected value."
                )
    ...
    def _validate_groups(
        ...
        if is_dummy_coded:
            ...
            if isinstance(x, pd.DataFrame):
                ...
                expected_col = [
                    sensitive_attribute.name
                    for sensitive_attribute in self.sensitive_map
                ]
                if observed_col != expected_col:
                    raise ValueError(
                        f"Expected columns ordered as: {expected_col} - instead received {observed_col}"
                    )
            else:
                ...
                if n_expected_col != n_expected_col:
                    raise ValueError(
                        f"Expected {n_expected_col} attributes but found {n_observed_col}."
                    )

    def _validate_trained(self):
        if not self._trained:
            raise NotFittedError(
                f"{self.__class__.__name__} is not fitted yet. Call 'fit' before using this method."
            )
